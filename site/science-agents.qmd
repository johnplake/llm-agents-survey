...
**NovaSky AI / UC Berkeley** · [sky.cs.berkeley.edu/project/skydiscover](https://sky.cs.berkeley.edu/project/skydiscover/)
...
**CISPO algorithm:** MiniMax's custom policy optimization combining elements of GRPO with stability constraints, enabling consistent convergence across heterogeneous environments.
...
Chinese LLM provider with the Kimi model series, known for **very long context windows** (up to 128k tokens) optimized for document-heavy tasks. Positioned as an alternative to OpenAI/Anthropic for scientific workflows requiring processing of large documents. Popular with researchers in China and East Asia.
...
### Evaluation: PaperBench

Evaluated on the **PaperBench** benchmark (tests ability to reproduce ML papers) and validated by human judges including paper authors (with author-released repos as ground truth). PaperCoder consistently outperforms strong baselines by substantial margins.
