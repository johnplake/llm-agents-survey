...
**Ecosystem explosion:** MCP spread faster than any comparable developer protocol. Within its first year, the official [MCP Registry](https://github.com/modelcontextprotocol/registry) — launched in preview in September 2025 — accumulated close to 2,000 entries. Unofficial directories like [mcp.so](https://mcp.so) indexed over 16,000 servers by early 2026. Major platforms adopted it rapidly: Visual Studio Code, Cursor, Claude, ChatGPT, and Windows 11 all announced MCP support within months of launch.

**Governance:** In December 2025, Anthropic donated MCP to the **Agentic AI Foundation (AAIF)**, a directed fund under the Linux Foundation co-founded by Anthropic, Block, and OpenAI. This moved MCP from a vendor-controlled spec to shared industry infrastructure.
...
A2A received a major update in **July 2025** adding gRPC transport support, signed Agent Cards (for authentication), and extended Python SDK coverage. The [A2A spec](https://github.com/a2aproject/A2A) is maintained as an open community project under the Linux Foundation LF AI & Data umbrella.
...
DSPy is particularly powerful for building agentic systems with retrieval, multi-hop reasoning, or tool use, where the interaction between components makes manual prompt tuning difficult. The [GitHub repo](https://github.com/stanfordnlp/dspy) has over 20,000 stars.
...
The **official MCP Registry** ([github.com/modelcontextprotocol/registry](https://github.com/modelcontextprotocol/registry)) launched in preview in September 2025. It serves as a canonical index where developers publish their MCP servers with structured metadata. The architecture supports a hierarchical system: a primary registry with sub-registries for specific communities, use cases, or enterprises.
...
The MCPTox benchmark has documented that tool poisoning attacks are alarmingly common in the MCP ecosystem. Real-world incidents have followed the same supply-chain attack pattern seen in traditional software:
...
- **Malicious updates:** a widely-installed MCP server package is updated with malicious code, instantly compromising all users (the mcp-remote npm package, with over 558,000 downloads, contained a critical vulnerability: CVE-2025-6514)
...
**AgentBound** is the first formal access-control framework for MCP servers. It combines:
...
- **AgentBound: Securing AI Agent Execution** (Bühler et al., 2025) — [arXiv:2510.21236](https://arxiv.org/abs/2510.21236) *(first declarative access-control framework for MCP servers; dataset of 296 top servers; 80.9% auto-generated policy accuracy)*
...
- **Personal LLM Agents: Insights and Survey about Capability, Efficiency and Security** (Liu et al., 2024) — [arXiv:2401.05459](https://arxiv.org/abs/2401.05459) *(on-device agent deployment constraints and design)*
